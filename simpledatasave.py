...
def datasave(file_name, file_value, file_directory = None, file_type = None):
    if file_directory == None:
        if file_type == None:
            f = open(f"./{file_name}.txt", "w", encoding="utf-8")
            f.write(str(file_value))
        else:
            f = open(f"./{file_name}.{file_type}", "w", encoding="utf-8")
            f.write(str(file_value))
    else:
        if file_type == None:
            ...
            f = open(f"{file_directory}/{file_name}.{file_type}", "w", encoding="utf-8")
